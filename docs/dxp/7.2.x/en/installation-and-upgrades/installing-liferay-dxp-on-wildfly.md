# Installing DXP 7.2 on Wildfly

Installing DXP 7.2 on Wildfly 11 takes three steps:

* [Installing dependencies to the application server](#installing-dependencies)
* [Configuring the application server for DXP 7.2](#configuring-wildfly)
* [Deploying the DXP 7.2 WAR file to the application server](#deploying-dxp-7.2)

| **Important:** Before installing DXP 7.2, please read [Optimizing the Application Server]() and [Preparing the Database]().

Now, [download the DXP 7.2 WAR and DependencyJARs](/docs/7-2/deploy/-/knowledge_base/d/obtaining-product#downloading-the-liferay-war-and-dependency-jars):

* DXP 7.2 WAR file
* Dependencies ZIP file
* OSGi Dependencies ZIP file

Note that [*Liferay Home*](https://help.liferay.com/hc/articles/360028712272-Liferay-Home) is the folder containing the Wildfly server folder. After installing and deploying DXP 7.2, the Liferay Home folder contains the Wildfly server folder as well as `data`, `deploy`, `logs`, and `osgi` folders. `$WILDFLY_HOME` refers to the Wildfly server folder. It is usually named `wildfly-[version]`.

## Installing Dependencies

DXP 7.2 depends on a driver for the database and the JARs in the Dependencies ZIP and OSGi Dependencies ZIP files.

1. Create the folder `$WILDFLY_HOME/modules/com/liferay/portal/main` if it does not already exist and extract the Dependencies ZIP JARs to it:
...
1. Create the file `module.xml` in the
    `$WILDFLY_HOME/modules/com/liferay/portal/main` folder and insert this configuration:
...
1. Create an `osgi` folder in your [Liferay Home](https://help.liferay.com/hc/articles/360028712272-Liferay-Home) folder. Extract the OSGi Dependencies ZIP file that you downloaded into the `[Liferay Home]/osgi` folder. 

    The `osgi` folder provides the necessary modules for DXP 7.2's OSGi runtime.

**Checkpoint:**

1. The following files are in the `$WILDFLY_HOME/modules/com/liferay/portal/main` folder:
...
1. The `osgi` folder has the following subfolders:
...
1. Note that DXP 7.2 creates these `osgi` subfolders the first time it starts:
...
## Running DXP 7.2 on Wildfly in Standalone Mode vs. Domain Mode
...
DXP 7.2 fully supports Wildfly in standalone mode but not in domain mode.

Administrators can run DXP 7.2 on Wildfly in domain mode, but this method is not fully supported. In particular, DXP 7.2's hot-deploy does not work with a managed deployment, since Wildfly manages the content of a managed deployment by copying files (exploded or non-exploded). This prevents JSP hooks and Ext plugins from working as intended. For example, JSP hooks don't work on Wildfly running in managed domain mode, since DXP 7.2's JSP override mechanism relies on the application server. Since JSP hooks and Ext plugins are deprecated, however, you may not be using them.

The command line interface is recommended for domain mode deployments.

| **Note:** This does not prevent DXP 7.2 from running in a clustered environment on multiple Wildfly servers. Administrators can set up a cluster of DXP 7.2 instances running on Wildfly servers running in standalone mode. Please refer to the [DXP 7.2 clustering articles](https://help.liferay.com/hc/articles/360029123831-Liferay-DXP-Clustering) for more information.

## Configuring Wildfly

Configuring Wildfly to run DXP 7.2 includes these things:

* Setting environment variables
* Setting properties and descriptors
* Removing unnecessary configurations

There are optional configurations in Wildfly to manage DXP 7.2's data source and mail session.

Start with configuring Wildfly to run DXP 7.2.
...
The prescribed script modifications are now complete for the DXP 7.2 installation on Wildfly.

### Database Configuration

The easiest way to handle database configuration is to let DXP 7.2 manage the data source. The [Basic Configuration](./preparing-the-database.md) page lets administrators configure DXP 7.2's built-in data source. If using the built-in data source, skip this section.
...
1. In a [`portal-ext.properties`](https://help.liferay.com/hc/articles/360028712292-Portal-Properties) file in the Liferay Home, specify the data source:
...
As with database configuration, the easiest way to configure mail is to let DXP 7.2 handle the mail session. If you want to use DXP 7.2's built-in mail session, skip this section and [configure the mail session](https://help.liferay.com/hc/articles/360029031591-Configuring-Mail) in the Control Panel.
...
1. In the [`portal-ext.properties`](https://help.liferay.com/hc/articles/360028712292-Portal-Properties) file in Liferay Home, reference the mail session:
...
1. Unzip the DXP 7.2 `.war` file into the `ROOT.war` folder.
...
| **Note:** After deploying DXP 7.2, you may see excessive warnings and log messages, such as the ones below, involving `PhaseOptimizer`. These are benign and can be ignored. Make sure to adjust your app server's logging level or log filters to avoid excessive benign log messages.
