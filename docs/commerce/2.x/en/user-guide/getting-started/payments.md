...
**Offline**: Liferay Commerce does not process the payment (for example, a buyer might pay in cash when picking up an order).

**Online Standard**: The payment is processed entirely by Liferay Commerce. This option is not available out-of-the-box but can be customized using the Payment Method extension point.

**Online Redirect**: Liferay Commerce passes information to a third-party payment processor, redirecting the buyer to the processor's website to complete the payment.
